...
namespace cudaMat
{
  ...
  struct SE3
  {
    ...
    __host__ __device__ __forceinline__ Type &operator()(int r, int c)
    {
      return data(r, c);
    }
    ...
    __host__ __device__ __forceinline__ float rotate_1_row(const float3 &p) const
    {
      return (data(0, 0) * p.x + data(0, 1) * p.y + data(0, 2) * p.z);
    }
    __host__ __device__ __forceinline__ float rotate_2_row(const float3 &p) const
    {
      return (data(1, 0) * p.x + data(1, 1) * p.y + data(1, 2) * p.z);
    }
    __host__ __device__ __forceinline__ float rotate_3_row(const float3 &p) const
    {
    ...
    __host__ __device__ __forceinline__
        float3
        translate(const float3 &p) const
    {
    ...
    __host__ __device__ __forceinline__
        float3
        getinvTranslation() const
    {
      SE3<Type> inv_ = this->inv();
      return inv_.getTranslation();
    }
    __host__ friend std::ostream &operator<<(std::ostream &out, const SE3 &m)
    {
      out << m.data;
      return out;
    }
  ...
  __host__ __device__ __forceinline__
      SE3<Type>
      operator*(const SE3<Type> &lhs, const SE3<Type> &rhs)
  {
    SE3<Type> result;
  ...
  __host__ __device__ __forceinline__
      float3
      operator*(const SE3<float> &se3, const float3 &p)
  {
